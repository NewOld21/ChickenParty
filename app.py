...
def home():
    # 페이지 번호를 쿼리 파라미터에서 가져옵니다. 기본값은 1.
    page = int(request.args.get('page', 1))
    per_page = 9  # 한 페이지에 보여줄 카드 수
    ...
    # 다음 페이지 여부 계산 (9개가 다 찼다면 다음 페이지가 존재)
...
def apply():
    if request.method == 'POST':
        ...
        if not title_receive:
            ...
            current_user = get_current_user()
            return render_template('apply.html', error_message='비밀번호를 입력해주세요!', user=current_user)
        ...
        invite = {
            ...
            'participants': [username],  #자기 자신을 참여자로 목록 추가
            "created_at": datetime.now(timezone.utc) + timedelta(hours=9)  # 한국 시간으로 설정
        }
...
def update():
    ...
    if request.method == 'POST':
        ...
        if not title_receive:
            ...
            return render_template('update.html', error_message='비밀번호를 입력해주세요!', user=current_user)
        limit_person_receive = request.form['limit_person_give']
        type_receive = request.form['type_give']
        note_receive = request.form['note_give']
        updated_data = {
